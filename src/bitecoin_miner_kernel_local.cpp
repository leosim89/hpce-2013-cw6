		...
		void MakeBid(
			...
			try{
				...
				if (period > threshold) {
					...
					std::string kernelSource=LoadSource("bitecoin_miner_kernel_local.cl");
						...
						for (unsigned k = 0; k < iterations; k++){
							for (unsigned i = 0; i < roundInfo->maxIndices; i++){
								for(unsigned x=0;x<8;x++){
									...
									proof2[k].limbs[x] = proof2[k].limbs[x]^point[(k*roundInfo->maxIndices+i)*8 + x];
								}
							}
						}
