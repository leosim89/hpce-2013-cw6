...
namespace bitecoin{
	...
	class EndpointClientV1: public EndpointClient
	{
		...
		void MakeBid(
			...
			double threshold_tbb = 3.0;
			double threshold_ocl = 10.0;
			try{
				...
				if (period > threshold_ocl) {
					...
					unsigned int iterations = maxcompunits*maxworkgroupsize*2;
					uint32_t *indices;
				...
				} else if (period > threshold_tbb) {
					Log(Log_Info, "TBB Branch");
				
					tbb::task_scheduler_init init(tbb::task_scheduler_init::automatic);
					fprintf(stderr, "Cores = %d\n", tbb::task_scheduler_init::default_num_threads());
					unsigned int iterations = tbb::task_scheduler_init::default_num_threads()*2;
